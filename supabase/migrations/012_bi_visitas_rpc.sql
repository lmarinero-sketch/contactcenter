...
BEGIN
  SELECT json_build_object(
    ...
    'heatmap', COALESCE((
      ...
      FROM (
        SELECT EXTRACT(ISODOW FROM fecha_hora_creacion AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires') as dia_semana, 
               EXTRACT(HOUR FROM fecha_hora_creacion AT TIME ZONE 'UTC' AT TIME ZONE 'America/Argentina/Buenos_Aires') as hora, 
               COUNT(*) as cantidad
        FROM salus_visitas
        WHERE (start_date IS NULL OR fecha_hora_creacion >= start_date)
          AND (end_date IS NULL OR fecha_hora_creacion <= end_date)
